...
usuarioModel.changeUser = function (login, pass, nombre, apell, mail, direccion, callback) {
    client.query('UPDATE usuario SET usr_pass = $1, usr_nombre = $2, usr_apell = $3, usr_mail = $4, usr_direccion = $5, WHERE usr_login = $6', [pass, nombre, apell, mail, direccion, login], function (error, results) {
        if (error) {
            console.log(error);
        } else {
            callback(null, results.rows);
